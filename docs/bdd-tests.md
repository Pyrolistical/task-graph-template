...
4. There can be **only one `When` per test**. The `When` represents the single behavior being tested.
...
12. **No `test.each`**. A table hides its values behind placeholders, leaving the Given/When/Then abstract.
