...
- the check runner has no agent and no slot: `checks.json` and `slots.json` are separate documents because the two have nothing to do with each other
...
- a failing check records nothing in the graph — it is a `fail` whose command, code and tail go to the worker's message queue; see [Failing forward](states.md#failing-forward)
