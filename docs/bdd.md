...
646 behaviours across 29 suites.
...
#### the model key drops the slot number and nothing else
...
#### a model setting the server cannot honour is refused
...
#### a write path is expanded from the home shorthand
...
#### every way out of a hold clears the reason
...
#### a project under home is keyed by its path below home
...
#### an id is written as six digits, whatever its number
...
#### a title of any shape survives being written and read
...
#### a claim without both its halves is refused
...
#### a check that is not a command line is refused
...
#### a task with no title is refused
...
#### a slot that is not inside a bash call is refused
...
#### an elapsed time changes units as it grows
...
#### a token count is only abbreviated once it passes a thousand
...
#### only a slot inside a bash call offers a button
