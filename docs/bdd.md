...
669 behaviours across 29 suites.
...
#### the slot %p belongs to the agent %p
...
#### a pool entry carrying a %p key is refused on load
...
#### the write path %p is expanded to %p
...
#### the project %p under home is keyed by %p
...
#### the project %p outside home is keyed by %p
...
#### the task number %p is written as the id %p
...
#### the title %p survives being written and read
...
#### the title %p is stored verbatim
...
#### a claim by %p with the pid %p is refused
...
#### a duration of %p milliseconds reads as %p
...
#### a slot that is %p offers no button
