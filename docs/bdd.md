...
711 behaviours across 29 suites.
...
#### the directory can put a task in every state there is
...
#### every allowed transition is written to the document
...
#### only the states an agent runs can be claimed
...
#### an agent that broke a rule is nudged four times
...
#### a review submit carries the findings it was given
...
#### an agent in any state can report itself blocked
...
#### every state loads its own extension of result tools
...
#### a closed task accepts no transition at all
...
#### the table of edges names every transition the machine allows
...
#### every transition the machine allows is one this suite exercises
...
#### every allowed transition lands in the state the machine promises

- **Given** a task sitting in each state the machine allows a transition from
- **When** each allowed transition is decided on that task
- **Then** each one lands where the machine says it does

#### every transition the machine does not allow is refused
...
#### a closed task has no transition left at all
...
#### an emoji, a cjk character and a keycap each take two columns
...
#### a joined family and a flag are each one two-column cluster
...
#### the width of a pane leaves room for the dividers between them
...
#### a switch shows its state by which side the knob sits on
