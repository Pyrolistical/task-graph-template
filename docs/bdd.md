...
977 behaviours across 30 suites.
...
## orchestrator/adapters/machine.test.ts

### Feature: applying the state machine to the task directory

#### the directory can walk a task into NEW
...
#### closing a task moves its document out of the way
...
#### closing a task lets everything waiting on it start
...
### Feature: claiming a task in the task directory

#### a task sitting in DESIGN can be claimed by an agent
...
#### a task sitting in PLAN can be claimed by an agent
...
#### a task sitting in WORK can be claimed by an agent
...
#### a task sitting in NEW cannot be claimed by an agent
...
#### a claim held by a live process is not released
...
#### a task another agent holds cannot be claimed
...
#### an agent cannot speak for a task nothing is holding
...
#### a rejected plan leaves the body for the planner

- **Given** a plan under review
- **When** the reviewer sends it back
- **Then** the task returns to planning with its body untouched

#### an accepted plan is written into the task body
...
#### a work review's findings are written into the body
...
### Feature: parking a task on a person
...
#### holding a task with no reason is refused
...
## orchestrator/adapters/prompts.test.ts
...
#### a title reading null survives being written and read
...
#### a slot still holding a dead process does not shield its task
...
#### agent_abort refuses a slot that is idle
