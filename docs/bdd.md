...
942 behaviours across 50 suites.

## orchestrator/agents/adapters/agents.test.ts
...
#### an agent runs at any time unless the pool says when
...
#### an hour written without its leading zero is refused
...
#### a schedule with no segments in it loads as none
...
#### an agent restricted to no role at all is refused
...
#### an agent has no slot limit unless the pool sets one
...
## orchestrator/agents/adapters/pi-agents.test.ts

### Feature: asking a provider whether it is up

#### a provider that lists its models is healthy
...
#### a provider that answers with an error is not healthy
...
#### a model pi does not know is an answer, not a crash
...
#### an api with no model list is an answer, not a crash
...
## orchestrator/agents/adapters/pi-process.test.ts
...
## orchestrator/agents/app/pool.test.ts
...
#### a slot inside a segment its agent declared is offered
...
#### a slot outside every segment its agent declared is held back
...
#### a slot outside its schedule reads as off schedule

- **Given** a pool whose only agent is allowed to run in half an hour's time
- **When** the pool is asked for its rows
- **Then** it reads as off schedule with its switch still on, because the agent is enabled

#### a slot outside its schedule never has its provider reached for
...
#### a slot holding a task keeps it when its schedule closes
...
#### a slot of an agent that is both off and outside its schedule reads as disabled
...
#### a slot whose agent asks for no health check is offered unasked

- **Given** a pool whose only agent leaves healthCheck off
- **When** the scheduler asks what is free
- **Then** the slot is offered and its provider was never reached for

#### a slot whose provider answers its health check is offered
...
#### a slot whose provider fails its health check is held back
...
#### the provider behind several slots is asked once
...
#### a provider that stays down is named once, not once a tick
...
#### a provider that comes back is said to be dispatchable again
...
#### an idle slot of a provider that is down reads as unreachable
...
#### a slot still holding a task keeps its own state while its provider is down
...
#### an idle slot of an agent that asks for no health check reads as idle
...
### Feature: what a running session has cost so far

#### the console is shown the price pi reports
...
#### a slot on an unpriced model shows what its power has cost
...
#### a slot with neither a price nor a meter costs nothing
...
#### a slot running a bash call has that turn aborted
...
#### a session that ends puts what pi charged for it on the task
...
#### a session on an unpriced model is billed for the power it drew
...
#### a resumed session is recorded as the same session, not a second one
...
#### a slot released before it opened a session bills the task nothing
...
#### a run outlives its slot, reading what it ran rather than the empty slot
...
### Feature: changing how many slots an agent runs with

#### every row says how many slots its agent is meant to have
...
#### a slot added at runtime takes the next free number and starts idle
...
#### a slot added at runtime is dispatchable
...
#### a slot added at runtime is spawned against its agent's settings
...
#### dropping a slot takes an idle one out of the pool at once
...
#### dropping a slot leaves the busy ones alone and takes the idle one

- **Given** a pool of two slots, the first of them busy on a task
- **When** the agent is set to one slot
- **Then** the idle slot is what leaves, and the task keeps running

#### with every slot busy the first one to go idle is the one that leaves
...
#### a slot waiting to be dropped is kept if the count goes back up
...
#### a number freed by a dropped slot is the one the next slot takes
...
#### the last slot of an agent cannot be dropped
...
#### a count above the agent's limit is refused

- **Given** a pool of one slot whose agent may run two
- **When** the agent is set to three slots
- **Then** it is refused, naming the limit the file declared

#### a count at the agent's limit is taken

- **Given** a pool of one slot whose agent may run two
- **When** the agent is set to two slots
- **Then** the limit is a ceiling to reach, not one to stop below

#### an agent the pool does not have is refused

- **Given** a pool of one agent
- **When** another agent's count is set
- **Then** it is refused by name, as enabling an unknown agent is

#### a slot dropped as it goes idle says so, so a reader sees it leave

- **Given** a pool of two busy slots the agent has been told to drop one of
- **When** its work ends
- **Then** the pool says which slot left and what the agent is down to

## orchestrator/agents/domain/backoff.test.ts

### Feature: waiting out a provider that is not answering

#### the first error waits the shortest step and counts an attempt
...
#### each further error doubles the wait and counts another attempt
...
#### a model that is still loading is waited on without spending an attempt
...
#### a wait is due once the clock reaches the time it named
...
#### the console is told when the retry is and which attempt it is
...
## orchestrator/agents/domain/health.test.ts

### Feature: where a provider is asked whether it is up

#### an openai-compatible provider is asked for its model list
...
#### anthropic is asked for the model list under its version prefix
...
#### a base url with a trailing slash does not double it
...
#### a keyless provider is asked without an authorization header

- **Given** a local server that takes no api key
- **When** the health check for that provider is worked out
- **Then** nothing is sent with it

#### an openai-compatible provider carries its key as a bearer token

- **Given** a provider configured with an api key
- **When** the health check for that provider is worked out
- **Then** the key is sent the way that api takes it

#### anthropic carries its key as an api key and names the version
...
#### google carries its key in the header that api reads

- **Given** the google provider configured with an api key
- **When** the health check for that provider is worked out
- **Then** the key is sent the way that api takes it

## orchestrator/agents/domain/schedule.test.ts

### Feature: the hours an agent is allowed to run

#### an agent with no schedule runs at any hour
...
#### an agent whose schedule names no segments runs at any hour
...
#### an agent is held before its segment opens

- **Given** an agent allowed to run through the working day
- **When** the minute before it opens is checked against it
- **Then** it is held

#### an agent is held in the minutes before its segment opens
...
#### an agent runs in a segment one minute wide
...
#### a segment ending before it starts runs late at night
...
## orchestrator/agents/policy/sizing.test.ts

### Feature: which slot numbers an agent takes as it grows

#### an agent growing from one slot takes the numbers above it
...
#### an agent takes the free numbers below its highest, before growing past it
...
#### an agent already at its count takes nothing
...
### Feature: which slots an agent drops as it shrinks

#### an agent drops its highest numbered slot first
...
#### every module belongs to a slice, bar the extensions and the test rig
...
#### no slice imports one further out than its own
...
#### a slice's use cases are private to it, and to the wiring
...
#### the layers that only decide reach for no effect at all
...
#### no test of a decision reaches for the filesystem
...
### Feature: what each slice keeps to itself

#### the task documents are reached through the task graph and nothing else
...
#### the queue that serialises edits is owned by the task graph alone

- **Given** every module that reaches for the edit queue
- **When** they are listed
- **Then** only the graph holds one, so no caller can mutate around it

#### nothing in the task slice points back at the server
...
#### the console reads the published views and nothing else of the server
...
#### every view schema the console parses is declared in the wire contract
...
### Feature: the drawing of it

#### the import graph in the docs is the one the modules make
...
## orchestrator/checks/adapters/check-runner.test.ts
...
## orchestrator/console/adapters/tui.test.ts
...
## orchestrator/console/domain/session.test.ts
...
## orchestrator/console/domain/text.test.ts
...
## orchestrator/console/policy/keys.test.ts
...
## orchestrator/console/policy/panes.test.ts
...
#### an unreachable slot says why it is holding no task
...
#### a slot outside its schedule says why it is holding no task
...
#### an idle slot facing a queue it cannot serve names the work it waits for
...
#### an idle slot open to several roles lists them in the order it declares
...
#### an idle slot the queue has work for shows no task
...
#### a disabled slot facing a queue it cannot serve still shows no task
...
#### a session that has spent money has its cost after the context
...
#### a session that has spent nothing shows no cost
...
#### a narrow pane clips the model rather than the switch, the slot or the state

- **Given** a busy pane in a thirty-column terminal
- **When** its header is drawn
- **Then** the model is what gives way, and the row still fills the pane

#### a slot of a provider that is down reads as unreachable, in red
...
### Feature: the slot count on a pane header

#### the only slot of an agent is drawn without a count
...
#### one slot of several says which of how many
...
#### a slot above its agent's count says so until it goes idle
...
#### the panes of an agent are numbered by where they are drawn
...
#### an agent below its limit is still offered the plus
...
#### a slot at its agent's limit takes no growing click
...
#### a pane the console has asked for reads as loading
...
#### each button asks for the count either side of the one drawn
...
#### a click on a button sends its count, and past the pair sends nothing
...
#### each pane's buttons sit in that pane's own columns
...
## orchestrator/console/policy/screen.test.ts
...
## orchestrator/console/policy/scroll.test.ts
...
## orchestrator/console/policy/toggles.test.ts

### Feature: switching an agent or the scheduler on and off

#### a click switches the agent before any view has agreed

- **Given** an agent the views still report as enabled
- **When** it is switched off
- **Then** every slot of it draws as off against a view that still says on

#### a view published before the server read the command does not flip the switch back
...
#### a command the server never applied resets the switch after two views
...
#### clicking again gives the switch a fresh two views
...
#### an agent no view reports is not held on the screen
...
#### the scheduler switch flips at once and resets the same way
...
#### a slot asked for is drawn at once as a pane still loading
...
#### a count the server never applied resets after two views

- **Given** an agent asked for a third slot
- **When** two views in a row still report two slots
- **Then** the first is taken as stale and the second gives the server back

#### a count the server applied is followed from then on
...
## orchestrator/kernel/adapters/paced.test.ts
...
## orchestrator/kernel/domain/exclusive-lock.test.ts
...
## orchestrator/kernel/domain/latch.test.ts
...
## orchestrator/kernel/domain/queue.test.ts

### Feature: edits applied one at a time

#### submitted work runs without anyone draining it
...
### Feature: what a task cost to get through
...
## orchestrator/prompting/adapters/prompt-files.test.ts
...
#### an aborted command is quoted back to the agent
...
## orchestrator/runtime/adapters/runtime.test.ts
...
## orchestrator/runtime/adapters/view-files.test.ts
...
## orchestrator/tasks/adapters/task.test.ts
...
## orchestrator/tasks/adapters/transitions.test.ts
...
## orchestrator/tasks/app/checker.test.ts
...
## orchestrator/tasks/app/dispatcher.test.ts

### Feature: what a dispatched agent is prompted with

#### a fresh session is told what was queued and then what the phase is
...
#### a resumed session is told what was queued and nothing else
...
## orchestrator/tasks/app/recovery.test.ts
...
#### what a reaped agent drew from the wall is billed to its task
...
#### a slot picked back up knows which state it is running
...
## orchestrator/tasks/app/settler.test.ts
...
### Feature: an agent whose command was aborted

#### the same session is told which command died, not thrown away
...
#### an agent aborted once too often has its task held
...
### Feature: a provider that is not answering

#### a turn that ends on a provider error leaves the slot waiting, not sleeping
...
#### the tick that finds the wait over prompts the same session again
...
#### a wait that is not over yet is left alone
...
## orchestrator/tasks/app/task-graph.test.ts

### Feature: one door onto the task documents

#### edits that arrive together are applied one after the other
...
## orchestrator/tasks/policy/inbox.test.ts
...
## orchestrator/tasks/policy/recent.test.ts

### Feature: the tasks the graph keeps in view

#### the task just touched goes to the front
...
## orchestrator/tasks/policy/settle.test.ts
...
#### a turn aborted on one command is prompted with that command
...
## orchestrator/vocabulary/assignment.test.ts
...
## orchestrator/vocabulary/costs.test.ts

### Feature: what a session cost

#### a provider that prices its tokens is taken at its word
...
#### a model with no price is billed for the power it drew
...
#### a resumed metered session adds to what it had already spent
...
#### a resumed priced session ignores what was carried
...
### Feature: how long a session ran

#### a session is timed to the second it held its slot
...
#### a second work session after a review is its own entry
...
#### what a state has spent is read off the last entry it holds
...
#### a state that has never run has spent nothing
...
#### a task in NEW submitted for working lands in WORK
...
#### a task in BLOCKED_DESIGN submitted for designing lands in DESIGN

- **Given** a task waiting to be designed, with nothing left to wait on
- **When** a submit_designing is decided on it
- **Then** the task lands in DESIGN

#### a task in BLOCKED_WORK submitted for working lands in WORK
...
#### a new task with dependencies is submitted into BLOCKED_DESIGN
...
#### a new task with dependencies submitted for working waits in BLOCKED_WORK
...
#### a blocked task submitted for another phase waits for that one instead
...
#### a held task that gained a dependency resumes into its phase's blocked state
...
#### holding a task carries the reason it was parked
...
## orchestrator/workspaces/adapters/git.test.ts
...
## orchestrator/workspaces/domain/workspace.test.ts
