...
- an idle slot is a row with no task, never a missing row: the view shows the whole pool as it stands
- `enabled` is the agent's toggle, not the slot's, so a draining agent reads `enabled: false` while its slot still reads `BUSY`
- `index` is the slot's own number and `total` its agent's [slot count](agents.md#slots), which are equal for a lone slot and differ the moment there are several; `index` above `total` is a slot [running out its task before it leaves](agents.md#changing-the-count-while-the-server-runs); `max` is the agent's [slot limit](agents.md#maxslots) and is absent when the file declared none
