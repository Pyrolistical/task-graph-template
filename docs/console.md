...
- disabled agents are drawn to the right of the running ones, and one `hide disabled` button is drawn across the middle of the whole block of them, however many there are
...
- with every agent in the pool disabled there is no button and nothing is collapsed, because hiding them would leave an empty screen — turning one back on brings the button back
