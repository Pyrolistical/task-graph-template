...
- `main/mcp.test.ts` drives the server as a **subprocess** over a real stdio client, and nothing a subprocess executes is instrumented: `mcp.ts` and `console.ts` are absent from the report, and manager-only paths in `app/server.ts` read as uncovered despite having tests
