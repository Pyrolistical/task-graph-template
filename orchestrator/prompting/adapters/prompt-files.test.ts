...
import { resultFromCall } from "../../agents/domain/results.ts";
...
describe("Feature: overriding the words an agent reads", () => {
  ...
  testInTempDirs("a project's own copy of a prompt wins", async () => {
    ...
    const fragment = (await PromptFiles.open(ORCHESTRATOR_DIR, dir)).fragment(
      "WORK",
    );
  ...
  testInTempDirs(
    ...
    async () => {
      ...
      const dir = await overrides({ prompts: "not a directory" });
      // When the prompts are loaded
      // Then it fails, rather than silently reading no prompts at all
      await expect(PromptFiles.open(ORCHESTRATOR_DIR, dir)).rejects.toThrow(
        /ENOTDIR/,
      );
    },
  );
  testInTempDirs(
    ...
    async () => {
      ...
      const prompts = await PromptFiles.open(ORCHESTRATOR_DIR, dir);
  ...
  testInTempDirs(
    ...
    async () => {
      ...
      const rendered = (await PromptFiles.open(ORCHESTRATOR_DIR, dir)).fragment(
        "check-failed",
        {
  ...
  testInTempDirs(
    ...
    async () => {
      ...
      await fs.writeFile(path.join(dir, "prompts", "WORK.md"), "two\n");
  ...
  testInTempDirs("reloading picks up an edited override", async () => {
    ...
    const cached = await prompts.reload();
  ...
  testInTempDirs(
    ...
    async () => {
      ...
      const dir = await overrides({ "prompts/WORK.md": "one\n" });
      const prompts = await PromptFiles.open(ORCHESTRATOR_DIR, dir);
      await fs.rm(path.join(dir, "prompts", "WORK.md"));
      // When the store is reloaded
      const cached = await prompts.reload();
