...
describe("Feature: what a review writes into the task body", () => {
  ...
  testInTempDirs("a review with no findings at all is refused", async () => {
    // Given finished work under review
    const { dir, id } = await toAgentReview();
    const before = await fs.promises.readFile(activeTaskPath(dir, id), "utf-8");
    ...
    expect(await fs.promises.readFile(activeTaskPath(dir, id), "utf-8")).toBe(
      before,
    );
  });
...
describe("Feature: closing a task", () => {
  testInTempDirs(
    ...
    async () => {
      ...
      expect(result.to).toBe("CLOSED");
      expect(await fs.promises.exists(activeTaskPath(dir, id))).toBe(false);
      expect(await fs.promises.exists(closedTaskPath(dir, id))).toBe(true);
...
describe("Feature: a transition that is refused writes nothing", () => {
  ...
  testInTempDirs(
    ...
    async () => {
      ...
      expect(await fs.promises.readFile(filePath, "utf-8")).toBe(before);
    },
  );
});
...
describe("Feature: what changes as a task walks the pipeline", () => {
  testInTempDirs(
    ...
    async () => {
      ...
      expect(await bodyOf(closedPath(run(dir, id, "submit")))).toBe(
        "\n# accepted",
      );
    },
  );
